...
Measured running `app/evaluate.py` for real, one case, against the real corpus: a single question took 609 seconds end to end (plan → research → critique → evaluate → auto-resumed human review), all on CPU-only Llama 3.1 8B. CLAUDE.md's non-functional requirements call for the evaluation harness to run in CI and compare agent versions over a versioned dataset; at this per-case latency, the 21-question dataset (`app/evaluation/dataset.json`) would take on the order of three and a half hours for a single full run, which is impractical both for local iteration and for a CI job.
