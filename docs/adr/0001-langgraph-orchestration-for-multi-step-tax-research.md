...
**Not yet wired into `build_graph`** — the guardrail exists and is unit-tested, but the graph itself does not yet call it as its first node. Tracked as the next step, not deferred scope: routing `START` through `guardrail` before `plan`, with a conditional edge to either `plan` (in scope) or a terminal node returning the `OutOfScopeResponse` directly, mirrors the existing `route_after_evaluate` conditional-edge pattern already used for human review.
