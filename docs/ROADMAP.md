...
- Audit trail persistence (see ADR-0004) — a new `audit_log` table on the same PostgreSQL instance, written from `app/main.py` after every completed run (including an out-of-scope rejection) via a pure `build_audit_record` and a real `PostgresAuditRepository`. Verified for real: an out-of-scope query produced a row with `in_scope=false`, null confidence, no LLM call needed.
